...
import { businessValidator } from 'helpers/formValidation/conditions.ts/validators';
...
const AdminBusinessesComponent = () => {
  ...
  const handleSubmitBusiness = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    let { isValid, errorMessage, formField } = validate(business, businessValidator);
    if (isValid) {
    ...
    // if needed, takes the saved element and populates the defaultvalue with these
  const populateVals = () => { }
